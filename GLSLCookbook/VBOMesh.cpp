...
VBOMesh::VBOMesh(GLSLProgram *program, const string &fileName, const string &vertexPos, const string &vertexNormal, 
				 const string &vertexTexcoords, const string &vertexTangents, const string &vertexEle, 
				 bool centre, bool loadTc, bool genTangents) : 
	mProgram(program), reCenterMesh(centre), loadTex(loadTc), genTang(genTangents){
    loadOBJ(fileName, vertexPos, vertexNormal, vertexTexcoords, vertexTangents, vertexEle);
}
...
void VBOMesh::loadOBJ(const string &fileName, const string &vertexPos, const string &vertexNormal, 
					  const string &vertexTexcoords, const string &vertexTangents, const string &vertexEle) {
    ...
    storeVBO(points, vertexPos, normals, vertexNormal, texCoords, vertexTexcoords, 
		tangents, vertexTangents, faces, vertexEle);
...
void VBOMesh::storeVBO( const std::vector<glm::vec3> &points, const std::string &vertexPos, 
					   ...
					   const std::vector<int> &elements, const std::string &vertexEle)
{
	...
	int posLoc = glGetAttribLocation(progHandle, vertexPos.c_str());
	...
	int normalLoc = glGetAttribLocation(progHandle, vertexNormal.c_str());
    glBindBuffer(GL_ARRAY_BUFFER, handle[bufIdx++]);
    glBufferData(GL_ARRAY_BUFFER, (3 * nVerts) * sizeof(float), n, GL_STATIC_DRAW);
    glVertexAttribPointer(normalLoc, 3, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );
    glEnableVertexAttribArray(normalLoc);  // Vertex normal

    if( tc != NULL ) {
		int texLoc = glGetAttribLocation(progHandle, vertexTexcoords.c_str());
        glBindBuffer(GL_ARRAY_BUFFER, handle[bufIdx++]);
        glBufferData(GL_ARRAY_BUFFER, (2 * nVerts) * sizeof(float), tc, GL_STATIC_DRAW);
        glVertexAttribPointer( texLoc, 2, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );
        glEnableVertexAttribArray(texLoc);  // Texture coords
    }
    if( tang != NULL ) {
		int tangentLoc = glGetAttribLocation(progHandle, vertexTangents.c_str());
        glBindBuffer(GL_ARRAY_BUFFER, handle[bufIdx++]);
        glBufferData(GL_ARRAY_BUFFER, (4 * nVerts) * sizeof(float), tang, GL_STATIC_DRAW);
        glVertexAttribPointer(tangentLoc, 4, GL_FLOAT, GL_FALSE, 0, ((GLubyte *)NULL + (0)) );
        glEnableVertexAttribArray(tangentLoc);  // Tangent vector
    }
